...
Keep this file canonical for repository rules. Load the linked resource index
only when you need upstream source URLs or citation checks.
...
| Need | Load |
| --- | --- |
| Choose an artifact type | [Decision model](#decision-model) |
| Check source locations | [Repository source boundaries](#repository-source-boundaries) |
| Apply artifact-specific rules | [Artifact specifications](#artifact-specifications) |
| Apply MartiX-only policy | [Repository overlays](#repository-overlays) |
| Check known caveats | [Open questions and cautions](#open-questions-and-cautions) |
| Validate changes | [Validation matrix](#validation-matrix) and [Standard validation commands](#standard-validation-commands) |
| Review upstream source links | [Custom AI artifact resources](./custom-ai-artifact-resources.md) |

## Resource index

The long upstream URL list lives in
[Custom AI artifact resources](./custom-ai-artifact-resources.md). Keep rule
decisions in this file; keep citation and discovery links in the resource index.
...
Run Markdown validation for the artifact docs:

```powershell
powershell -ExecutionPolicy Bypass `
  -File .\plugins\martix-markdown-automation\hooks\markdown-check.ps1 `
  -CheckOnly `
  -Path .\docs\policy\custom-ai-artifact-rules.md,`
    .\docs\policy\custom-ai-artifact-resources.md
```
