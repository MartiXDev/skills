...
This guide describes how MartiX skills and plugins should be used by agents
without wasting premium model requests, tokens, memory, or parallel-agent
capacity. It combines execution profiles with the repository's model-routing
policy so those vocabularies do not drift apart.

Use this document when adding or reviewing package metadata, evals, templates,
plugin bundle instructions, and `/fleet` work plans.
...
routing. Do not add `"model": "cheap"` or similar aliases to `plugin.json` or
to Agent Skills `SKILL.md` front matter. Those locations do not select models
...
Use tier names in repository-owned metadata and evals. Do not duplicate vendor
model names across skill packages.

| Artifact | Responsibility |
| --- | --- |
| `docs\guides\execution-and-routing.md` | Full tier, budget, task-classification, routing, and parallel-safety vocabulary. |
...
to this guide instead of embedding the full routing rules.
